...
async function main () {
    ...
    await Promise.all([
        ...
        Deno.readTextFile('./tmp-package.json')
            ...
            .then(str => str.replace('0.0.0-VERSION', pkg.version))
...
const replace_all: (old: string | RegExp, next: string) => (origin: string) => string
= (old, next) => origin => origin.replaceAll(old, next);
